...
const UpdateServiceRequestStatus = async (
	...
	if (serviceRequest.status !== "PENDING") {
		throw new AppError(
			HttpStatus.BAD_REQUEST,
			"Only pending service requests can be reviewed.",
		);
	}
...
const assignServiceRequest = async (
	...
	if (serviceRequest.status !== "CONFIRMED") {
		throw new AppError(
			HttpStatus.BAD_REQUEST,
			"Only confirmed service requests can be assigned.",
		);
	}
...
const updateServiceRequestInProgressStatus = async (
	...
	if (!serviceRequest) {
		throw new AppError(
			...
			"Service request not found or not assigned to you.",
		);
	}
	...
	if (
		...
		throw new AppError(
			HttpStatus.BAD_REQUEST,
			"In-progress service requests can only be completed.",
		);
	}
